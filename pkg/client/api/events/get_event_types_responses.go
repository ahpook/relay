...
type GetEventTypesOKBody struct {
	...
	Types []*models.EventType `json:"types"`
}
...
func (o *GetEventTypesOKBody) validateTypes(formats strfmt.Registry) error {

	if swag.IsZero(o.Types) { // not required
		return nil
	}

	for i := 0; i < len(o.Types); i++ {
		if swag.IsZero(o.Types[i]) { // not required
			continue
		}

		if o.Types[i] != nil {
			if err := o.Types[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("getEventTypesOK" + "." + "types" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
